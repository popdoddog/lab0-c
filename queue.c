...
bool q_delete_mid(struct list_head *head)
{
    ...
    for (; right != left && right->next != left;
         right = right->prev, left = left->next)
        ;
    element_t *obj = list_entry(left, element_t, list);
    list_del(&obj->list);
    q_release_element(obj);
    return true;
}
